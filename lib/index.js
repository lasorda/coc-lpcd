...
function provideCompletionItems(document, position) {
  ...
  if (filename in completionCache && filename in completionCacheTime && Date.now() / 1e3 - completionCacheTime[filename] < 1) {
...
function getRangeofWordInFileLine(filename, line, word) {
  let res = {start: {line, character: 0}, end: {line, character: 0}};
  let filelines = [];
...
function provideDefinition(document, position) {
  ...
  reg = /#include\s+?<([\w|\/|\.]+)*?>(\s+)?/;
  if (reg.test(lineText)) {
    let exec_result2 = reg.exec(lineText);
    if (exec_result2) {
      let uri = path.resolve(inc, exec_result2[1]);
