...
function parse(filename, symbolInfo) {
  ...
  let fileSymbol = {defined: [], include: [], variable: [], func: [], childFileSymbol: new Map(), lineno: 0};
  let localArgs = [];
      ...
      switch (lineSymbol.op) {
        ...
        case 2:
          ...
          if (hasArgs) {
            ...
            targetSymbol.defined.push({name: define.substring(0, hasArgs), line: lineSymbol.lineno, args});
          } else {
            if (spacePos < 0) {
  ...
  fileSymbol.childFileSymbol.forEach((value, key, m) => {
