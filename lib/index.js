...
var require_utils = __commonJS((exports2) => {
  ...
  var getASCIIChar = function(str) {
    var num = exports2.octal2int(str);
...
var require_gets = __commonJS((exports2, module2) => {
  var fs2 = require("fs");
  var BUFSIZE = 256;
  var buf = Buffer.alloc ? Buffer.alloc(BUFSIZE) : new Buffer(BUFSIZE);
  var bytesRead;
  module2.exports = function() {
    var fd = process.platform === "win32" ? process.stdin.fd : fs2.openSync("/dev/stdin", "rs");
    bytesRead = 0;
    try {
    ...
    var content = buf.toString(void 0, 0, bytesRead - 1);
    return content;
  };
});

// node_modules/scanf/lib/scanf.js
var require_scanf = __commonJS((exports2, module2) => {
  var utils = require_utils();
  var gets = require_gets();
  var input = "";
  var stdin_flag = true;
  exports2.throw = true;
  var scanf2 = module2.exports = function(format) {
    ...
    var json_flag = false, count = 0, keys = Array.prototype.slice.call(arguments, 1);
    if (!this.sscanf) {
      if (!stdin_flag)
        input = "";
      stdin_flag = true;
  ...
  module2.exports.sscanf = function(str, format) {
    ...
    input = str;
    stdin_flag = false;
    return scanf2.apply({sscanf: true}, Array.prototype.slice.call(arguments, 1));
  };
  ...
  var getHexFloat = function(pre, next) {
    ...
    var integer = parseInt(sign + sint + (spoint !== void 0 ? spoint.slice(1) : ""), 16);
    var exponent = parseInt(sexp.slice(1), 16) - 4 * (spoint !== void 0 ? spoint.length - 1 : 0);
  ...
  var getLine2 = function(pre, next) {
...
var require_scanf2 = __commonJS((exports2, module2) => {
  module2.exports = require_scanf();
});
...
var cocNvim = __toModule(require("coc.nvim"));
var child_process = __toModule(require("child_process"));
var fs = __toModule(require("fs"));
var path = __toModule(require("path"));
var vslp = __toModule(require_main3());
var scanf = __toModule(require_scanf2());
var import_file_uri_to_path = __toModule(require_src());
var cocLpcdConfig = cocNvim.workspace.getConfiguration("coc-lpcd");
var workspaceStr = cocLpcdConfig.get("workspace", "newtxii");
var complieCommand = cocLpcdConfig.get("complie", "lpc_compile");
var efuncObjects = cocLpcdConfig.get("efunc", ["/sys/object/simul_efun.c", "/etc/efun_define.c"]);
var projectFolder = "";
var inc = "";
var logger;
...
function getFileRelativePath(uri) {
  return path.relative(projectFolder, (0, import_file_uri_to_path.default)(uri));
}
function InitProjectFolder() {
  ...
  if (pos >= 0) {
    ...
    inc = path.resolve(projectFolder, cocLpcdConfig.get("include", "inc"));
  }
...
function complie(filename) {
  try {
    child_process.execSync(`cd ${projectFolder}; mkdir -p log && touch log/debug.log`, {shell: "/bin/bash", stdio: "pipe"});
...
function parse(filename, symbolInfo) {
  ...
  lineInfo.forEach((line) => {
    if (line.length == 0)
      return;
    const lineSymbol = scanf.sscanf(line, "%d %s %d %S", "op", "filename", "lineno", "detail");
...
function attachComment(fileSymbol) {
  ...
  for (const e of all) {
    ...
    if (commentIdx[e.line]) {
      for (const idx2 of commentIdx[e.line]) {
        res.push(fileSymbol.comment[idx2].detail);
      }
    }
    if (commentIdx[e.line - 1]) {
      for (const idx2 of commentIdx[e.line - 1]) {
        res.push(fileSymbol.comment[idx2].detail);
      }
    }
    if (res.length) {
...
function generateFileSymbol(filename) {
  let fileSymbol = {defined: [], include: [], variable: [], func: [], childFileSymbol: {}, lineno: 0, comment: [], filename};
  ...
  if (reg.test(lineText)) {
    ...
    if (exec_result) {
      ...
      if (!exec_result[2].startsWith("/")) {
        target = path.resolve(path.dirname((0, import_file_uri_to_path.default)(document.uri)), ...dir);
      } else {
...
function getWordRangeAtPosition(document, position) {
  ...
  while (left >= 0 && (line[left] >= "a" && line[left] <= "z" || line[left] >= "A" && line[left] <= "Z" || line[left] >= "0" && line[left] <= "9" || line[left] == "_" || line[left] == "/"))
    left--;
  left++;
  while (right < line.length && (line[right] >= "a" && line[right] <= "z" || line[right] >= "A" && line[right] <= "Z" || line[right] >= "0" && line[right] <= "9" || line[right] == "_" || line[right] == "/"))
    right++;
  return {start: {line: lineNumber, character: left}, end: {line: lineNumber, character: right}};
}
function searchInLine(line, word) {
...
var fileContextCache = {};
function getRangeofWordInFileLine(filename, line, word) {
  const res = {start: {line, character: 0}, end: {line, character: 0}};
  let filelines = [];
...
function provideDefinition(document, position) {
  ...
  do {
    if (exec_result != null && exec_result[1] != null && exec_result["index"] + exec_result[1].length + 2 <= position.character && exec_result["index"] + exec_result[1].length + word.length >= position.character - 1) {
      let from = "";
      if (exec_result[1] == "this_object()") {
      ...
      if (!fs.existsSync(path.resolve(projectFolder, from)))
        return;
  ...
  reg = /#include\s+?<([\w|/|.]+)*?>(\s+)?/;
  if (reg.test(lineText)) {
    const exec_result2 = reg.exec(lineText);
    if (exec_result2) {
      const uri = path.resolve(inc, exec_result2[1]);
  ...
  for (const local of getLocalVariable(filename, position.line)) {
    if (local.name == word) {
      ...
      return {
        uri: document.uri,
        range: getRangeofWordInFileLine((0, import_file_uri_to_path.default)(document.uri), local.line - 1, local.name)
      };
    }
  }
  for (const variable of getGlobalVariable(filename, position.line, true)) {
  ...
  debug(getMacroDefine(filename, position.line, true));
  for (const define of getMacroDefine(filename, position.line, true)) {
    debug(define.filename, define.name);
    if (define.name == word) {
      hoverSymbol = define;
      return {
...
var fileModifyTime = {};
function isFileChange(filename) {
  const t = fs.statSync(path.resolve(projectFolder, filename)).mtime.getTime();
  return fileModifyTime[filename] != t;
}
function isFileSymbolChange(fileSymbol) {
  for (const file in fileSymbol.childFileSymbol) {
    const childSymbol = fileSymbol.childFileSymbol[file];
    if (isFileSymbolChange(childSymbol))
      return true;
  }
  return isFileChange(fileSymbol.filename);
}
function updateModifyTime(fileSymbol) {
  for (const file in fileSymbol.childFileSymbol) {
    const childSymbol = fileSymbol.childFileSymbol[file];
    updateModifyTime(childSymbol);
  }
  debug("xx", projectFolder, fileSymbol.filename, path.resolve(projectFolder, fileSymbol.filename));
  fileModifyTime[fileSymbol.filename] = fs.statSync(path.resolve(projectFolder, fileSymbol.filename)).mtime.getTime();
}
async function activate(context) {
  logger = context.logger;
  InitProjectFolder();
  const selector = [{language: "lpc"}];
  context.subscriptions.push(cocNvim.languages.registerCompletionItemProvider("coc-lpcd", "LPC", "lpc", {provideCompletionItems}, ["/", ">", "<"]), cocNvim.languages.registerDefinitionProvider(selector, {provideDefinition}), cocNvim.languages.registerDocumentSymbolProvider(selector, {provideDocumentSymbols}), cocNvim.languages.registerHoverProvider(selector, {provideHover}));
}
