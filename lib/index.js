...
var documentSymbolCache = {};
var documentSymbolCacheTime = {};
function provideDocumentSymbols(document, token) {
  let filename = getFileRelativePath(document.uri);
  if (filename in documentSymbolCacheTime && Date.now() / 1e3 - documentSymbolCacheTime[filename] < 2) {
    return documentSymbolCache[filename];
  }
  let output = [];
  if (!complie(filename)) {
    if (filename in documentSymbolCache)
      return documentSymbolCache[filename];
    return output;
  }
  ...
  documentSymbolCacheTime[filename] = Date.now() / 1e3;
  return output;
}
