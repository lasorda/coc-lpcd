...
function getFileAndDir(dirPath) {
  ...
  for (let i = 0; i < files.length; ++i) {
    ...
    if (isFile && (filedir.search("\\.c") != -1 || filedir.search("\\.h") != -1)) {
      ...
      output.push({
        label: filedir,
        kind: import_coc.CompletionItemKind.Folder,
...
async function activate(context) {
  import_coc.window.showMessage(`coc-lpcd works!`);
  ...
  context.subscriptions.push(import_coc.languages.registerDefinitionProvider([{language: "lpc"}], {provideDefinition}));
}
